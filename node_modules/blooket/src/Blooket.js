...
class Blooket extends EventEmitter {
    ...
    async createGame(hostName, isPlus, qSetId, t_a /* t = Time | a = Amount */, gameMode, cookie) {
        const dateISOString = new Date().toISOString();

        const liveData = await liveCred(hostName, isPlus, qSetId, dateISOString, t_a, gameMode, cookie);
    ...
    async getAccountData(cookie) {

        const response = await axios(utils.links.verifyAcc, {
            headers: {
                Referer: 'https://www.blooket.com/',
                cookie: cookie
            },
    ...
    async spamPlayGame(setId, name, cookie, amount) {
        for (let i = 0; i < amount; i++) {
            try {
    ...
    async favoriteSet(setId, name, cookie) {
        const checkIfFavorited = await isFavorited(setId, cookie);
    ...
    async createSet(author, desc, isPrivate, title, cookie) {
    ...
    async addTokens(tokenAmount, xpAmount, name, cookie) {
    ...
    async deleteHomework(setId, cookie) {
        try {
    ...
    async sellBlook(blook, name, numSold, cookie) {
